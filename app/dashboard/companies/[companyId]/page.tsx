...
export default async function CompanyDetailPage({
  ...
  const returnPath = `/dashboard/companies/${company.id}`
    ...
    <>
      ...
      <div className="app-page-header">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="app-page-title">{companyName}</h1>
            <span className={statusBadgeClass(isActive)}>
              {isActive ? 'Active' : 'Inactive'}
            </span>
          </div>
          <p className="app-page-subtitle">
            Manage this company and review trips linked by company ID.
          </p>
        </div>
      ...
      <section className="app-section-card mb-5 space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Update the company name used in trip forms and filters.
          </p>
        </div>

        <form action={updateCompany} className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          <input type="hidden" name="id" value={company.id} />
          <input type="hidden" name="return_path" value={returnPath} />
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Trips linked to this company through <code>trips.company_id</code>.
          </p>
        </div>
              ...
              {trips.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-5 text-gray-700">
                    No trips linked to this company.
                  </td>
                </tr>
              ) : (
                trips.map((trip) => {
